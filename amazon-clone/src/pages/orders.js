  ...
  return (
    <div>
        ...
        <main className='max-w-screen-lg mx-auto p-10'>
            <h1 className='text-3xl font-medium border-b mb-2 pb-1 border-yellow-400'>Your Orders</h1>
        {session ? (
            <h2>
            ...
            <div className='mt-5 space-y-4'>
                {orders?.map(({id, amount, items, timestamp, images}) => (
                    <Order key={id} id={id} amount={amount} items={items} timestamp={timestamp} images={images}/>
                ))}
            </div>
